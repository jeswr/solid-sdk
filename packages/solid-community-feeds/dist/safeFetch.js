...
function isBlockedIp(host) {
    ...
    if (isIpv4(h)) {
        ...
        if (a >= 224)
            return true; // multicast + reserved 224.0.0.0/3
        return false;
    }
    // IPv6 (lower-cased)
...
export function assertSafeUrl(rawUrl) {
    ...
    if (isIpLiteral(url.hostname)) {
        if (isBlockedIp(url.hostname)) {
            throw new SafeFetchError("blocked-host", `refusing to fetch a private/reserved address: ${url.hostname}`);
        }
    }
    else if (isBlockedHostname(url.hostname)) {
        throw new SafeFetchError("blocked-host", `refusing to fetch a local/internal hostname: ${url.hostname}`);
    }
    return url;
}
/**
...
export async function safeFetch(rawUrl, init, opts = {}) {
    ...
    try {
        ...
        try {
            ...
            throw new SafeFetchError("network", `network error: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (res.status >= 300 && res.status < 400) {
            throw new SafeFetchError("redirect", `refusing to follow redirect (status ${res.status})`, res.status);
        }
        if (!res.ok) {
        ...
        if (Number.isFinite(declared) && declared > maxBytes) {
            throw new SafeFetchError("too-large", `declared body ${declared} bytes exceeds cap of ${maxBytes}`);
        }
...
async function readStreamCapped(body, maxBytes, controller, timeoutMs) {
    const decoder = new TextDecoder("utf-8");
    const parts = [];
    let total = 0;
    try {
        for await (const chunk of body) {
            const byteLen = typeof chunk === "string" ? Buffer.byteLength(chunk, "utf8") : chunk.byteLength;
            total += byteLen;
            if (total > maxBytes) {
...
async function readTextCapped(res, maxBytes, controller, timeoutMs) {
    let text;
    try {
        text = await res.text();
    }
