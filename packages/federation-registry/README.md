...
npm run api:check   # fail if the public API drifted from the committed etc/federation-registry.api.md snapshot
npm run api:report  # regenerate etc/federation-registry.api.md after an INTENDED public-API change
```
...
[`etc/federation-registry.api.md`](./etc/federation-registry.api.md) is the
single-file, reviewable snapshot of this package's public API, generated by
[API Extractor](https://api-extractor.com/). It is **not** shipped in the
published tarball (it is absent from `files` in `package.json`) — it exists only
to make a public-API change a reviewable diff. `npm run api:check` fails the
build if the surface drifts from the committed snapshot (and, with
`ae-forgotten-export` set to `error`, a public export whose type is not itself
exported can never silently slip in); regenerate it with `npm run api:report`
after an intended API change and commit the result alongside your `src/` edit.
