...
//
// Public domain types for the federation registry client: the plain-object views
// over a fedreg:Registry / fedreg:Membership / fedreg:StorageDescription graph
// that build/parse/verify exchange. The RDF itself is read/written through
// @rdfjs/wrapper typed accessors (never these objects directly) — these are the
// SDK's ergonomic surface.
...
export type RegistryIssueCode =
  ...
  | "multiple-registries"
  | "no-membership"
  | "membership-missing-app"
  | "membership-multiple-apps"
  | "membership-missing-status"
  | "membership-multiple-statuses"
  | "unknown-status"
  | "membership-missing-asserted-by"
  | "invalid-term-type"
  | "no-storage-description"
  | "storage-missing-accepts-spec"
  | "fetch-failed"
  | "parse-failed";
