 ...
 */
export async function driveHeadlessOidc(
  ctx: HeadlessOidcContext,
  authUrl: string,
  listener: Pick<LoopbackListener, "redirectUri" | "waitForCode">,
  ...
  for (let step = 0; step < 6; step += 1) {
    ...
    if (p.prompt === "login" || p.prompt === "select_account") {
      ...
      absorb(r);
      if ((await follow(((await r.json()) as { location?: string }).location)) === "done") break;
