...
export function parseContentLine(line) {
    ...
    for (let i = 1; i < parts.length; i++) {
        ...
        const eq = p.indexOf("=");
        if (eq < 0)
            continue; // malformed param — skip, do not throw
    ...
    for (const line of lines) {
        ...
        if (parsed.name === "BEGIN") {
            if (stack.length >= MAX_NESTING)
                continue; // refuse to nest deeper
        ...
        if (parsed.name === "END") {
            ...
            if (parent)
                parent.components.push(finished);
            else
                top.push(finished);
            continue;
        }
        ...
        const current = stack[stack.length - 1];
        if (current)
            current.properties.push(parsed);
 ...
 */
export function findComponents(roots, name) {
