 ...
 */
function addDate(quads, subjectTerm, prop, predicate) {
    if (!prop || typeof prop.value !== "string")
        return;
    const parsed = parseICalDate(prop.value, isDateValue(prop));
    if (!parsed)
        return;
    quads.push(quad(subjectTerm, namedNode(predicate), literal(parsed.value, namedNode(parsed.datatype))));
 ...
 */
export function veventToEvent(component, options) {
    ...
    const result = uid !== undefined ? { subject, quads, uid } : { subject, quads };
    return result;
}
// --- vCard → ContactData ---
/**
...
function encodeMailtoLocal(local) {
    let out = "";
    for (const ch of local)
        out += MAILTO_LOCAL_SAFE.test(ch) ? ch : pctEncodeChar(ch);
    return out;
}
/**
 ...
 */
export function vcardToContact(component, options = {}) {
    ...
    for (const e of getProperties(component, "EMAIL")) {
        ...
        if (mailto && !emails.includes(mailto))
            emails.push(mailto);
    }
    const phones = [];
