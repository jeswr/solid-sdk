...
/**
 * check-dist-fresh — guard against the COMMITTED `dist/` drifting from source.
 *
 * `dist/` is committed (not gitignored) so the package installs directly from a
 * GitHub branch without a build step AND so n8n's community-node loader finds the
 * built node + credential — consumers run under `ignore-scripts=true` and never
 * execute this package's `prepare`/`build`. That only stays correct if the
 * committed artifact matches the source. This script rebuilds into a scratch dir
 * (the SAME bundled build as `npm run build`) and diffs the emitted JavaScript +
 * declarations against the version of `dist/` at git HEAD.
 *
 * Why compare against git HEAD, not the working-tree `dist/`: `npm run build`
 * overwrites the working-tree `dist/`. Comparing against the blobs at
 * `HEAD:dist/<path>` makes the check independent of whether `build` ran first.
 *
 * It deliberately ignores `*.map` sourcemaps (vary with absolute paths) and
 * `*.svg` icons (copied verbatim, not emitted). Code (`.js`) and types (`.d.ts`)
 * matter.
 *
 ...
 * keyed by their path RELATIVE to `dist/`. Uses `git ls-tree` so it reads the
 * COMMITTED tree, never the working copy.
 */
...
let scratch;
try {
  scratch = mkdtempSync(join(tmpdir(), "n8n-nodes-solid-dist-"));
  ...
  if (drift.length > 0) {
    console.error("committed dist/ is out of sync with source:");
  ...
  console.log(`committed dist/ matches source (${freshFiles.size} artifacts).`);
} finally {
  if (scratch) {
