...
// (Ported from @jeswr/solid-elements `src/login-controller.ts` — the seam types are
// extracted here so the framework-free auth core has no web-components dependency.)
//
// The SolidAuthController seam — the structural contract between an app / a UI
// component and the security-critical auth machinery. The two fetches are the
...
//   - `publicFetch` — the PRISTINE native fetch captured BEFORE anything patched
//     `globalThis.fetch`. Carries NO credentials and never upgrades on a 401.
//     Use for foreign-origin / public reads so a session token can never leak
//     cross-origin.
// Before login both are the pristine fetch (there is no session to bind).
//
// This module is PURE TYPES + one tiny pure helper — no auth dep, no DOM dep.
...
export type RedirectOutcome =
  | { outcome: "completed"; webId: string }
  | { outcome: "redirecting" }
...
export interface SolidAuth extends SolidAuthController {
  ...
  //
  // The redirect counterpart to the popup {@link SolidAuthController.login}: it
  // survives a full-page navigation by persisting its in-between state to
  // sessionStorage. Most apps call ONLY {@link handleRedirect} (on load) — it drives
  // the whole lifecycle; the two lower-level methods are for bespoke wiring.
   ...
   */
  beginRedirectLogin(options: BeginRedirectLoginOptions): Promise<{ authorizationUrl: string }>;
