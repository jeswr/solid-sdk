...
// (Ported verbatim — plus the pristine-brand integration — from
// @jeswr/solid-elements `src/auth/proactive-fetch.ts` (Opus 4.8-authored original,
// task #123); this package is now its home, solid-elements will re-export.)
//
...
import { computeAllowedOrigins, isOriginAllowed, isUseDpopNonceChallenge } from "./controller.js";
import { brandFetchWrapper, resolvePristineFetch } from "./pristine.js";
...
export function installProactiveAuthFetch(
  ...
  // PRISTINE RESOLUTION (see ./pristine.js): the snapshot — injected or read from
  // the global — is UNWRAPPED through this package's wrapper brand, so installing
  // over a global that one of OUR wrappers already patched (a second install path,
  // a second bundle copy) anchors on the TRUE pristine base instead of chaining.
  const rawGlobal = globalThis.fetch;
  const resolvedGlobal = resolvePristineFetch(rawGlobal);
  const pristineFetch =
    options.pristineFetch !== undefined
      ? resolvePristineFetch(options.pristineFetch)
      : resolvedGlobal === rawGlobal
        ? rawGlobal.bind(globalThis)
        : resolvedGlobal;
  const config: ProactiveFetchConfig = options.isSuperseded
    ? { isSuperseded: options.isSuperseded }
    : {};
  // The single mutable cell the wrapper reads on every request. ALL FOUR state
  // fields are carried (the solid-elements original dropped the two optional
  // #123 PM-parity seams here + in setState, making them unreachable through the
  // installer — fixed in this extraction; a safe superset, they default off).
  const state: ProactiveFetchState = {
    ...
    ...(options.initial?.issuerOrigins !== undefined
      ? { issuerOrigins: options.initial.issuerOrigins }
      : {}),
    ...(options.initial?.canAttachNonInteractively !== undefined
      ? { canAttachNonInteractively: options.initial.canAttachNonInteractively }
      : {}),
  };
  // The wrapper is BRANDED with its pristine base so resolvePristineFetch can
  // recover it (the login-stall unrepresentability guarantee — ./pristine.js).
  const wrapper: typeof fetch = brandFetchWrapper(
  ...
  const install: ProactiveFetchInstall = {
    setState(next) {
      ...
      // The optional seams are REPLACED (not merged): an update that omits one
      // clears it, so a stale issuer set / liveness gate never outlives a
      // re-login that no longer supplies it (fail-safe: absent ⇒ seam off).
      if (next.issuerOrigins !== undefined) state.issuerOrigins = next.issuerOrigins;
      else delete state.issuerOrigins;
      if (next.canAttachNonInteractively !== undefined) {
