...
//
// Local STRUCTURAL re-declarations of @jeswr/solid-session-restore's store
// contract. That package is OFF-NPM (github:-installed) and its compiled code is
// INLINED into this package's committed dist — so a GitHub consumer of
// @jeswr/solid-auth-core does NOT have it installed, and our emitted `.d.ts`
// must never reference it (check-dist-fresh enforces this). These shapes are
// byte-for-byte the same structural contracts, so values flow to/from the real
// package without casts; if the upstream shapes ever change, the typecheck
// against the (dev-installed) real package breaks loudly here.
