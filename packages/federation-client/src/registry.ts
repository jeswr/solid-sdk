...
//
// Consume a federation **Catalogue / Registry** (the fedreg: layer) from the
// federation client. Two capabilities, mirroring the registry's two questions
// (see @jeswr/federation-registry):
//   1. discoverFromRegistry(registryUrl) — the REGISTRY-ASSERTED memberships: which
//      apps a registry lists as members (distinct from a self-asserted fedapp:App,
//      which `list()` discovers). Each carries the registry's lifecycle `status` and
//      `assertedBy` authority.
//   2. resolveStorageSpecVersion(storageUrl) — a storage's advertised client-client
//      spec-VERSIONS (fedreg:acceptsSpec) + supported sectors, for schema-migration
//      coordination (which version may an app safely write?).
//
// RDF discipline: we DO NOT re-parse the registry's fedreg: RDF — we delegate to
// @jeswr/federation-registry's published typed accessors (parseRegistry / parseStorage
// / acceptsSpec / unsupportedSpecs) and re-shape their results into this client's
// existing return conventions. The registry owns its model; this module is a thin,
// SSRF-guarded consumer of it.
//
// SSRF: the registry/storage URL is a user/config-supplied remote origin, so every
// fetch is wrapped by the guard in ./ssrf.ts (https-only, private/loopback/metadata
// blocked, redirects re-validated, body/time capped) BEFORE it reaches the registry's
// fetch path. The guard is composed with any caller-supplied `fetch` (auth fetch).

import {
  type Membership,
  type MembershipStatusName,
  type RegistryIssue,
  acceptsSpec as registryAcceptsSpec,
  parseRegistry as registryParseRegistry,
  parseStorage as registryParseStorage,
  unsupportedSpecs as registryUnsupportedSpecs,
  type StorageDescription,
  TRUSTED_STATUS,
} from "@jeswr/federation-registry";
import { createGuardedFetch, type GuardOptions } from "./ssrf.js";
...
): Promise<RegistryDiscovery> {
  const fetchImpl = guardedFetchFor(options);
  // parseRegistry fetches + parses the registry document, verifies each membership,
  // AND preserves document-level issues (fetch/parse/no-registry). We delegate all
  // parsing to its typed fedreg: accessors, then re-shape into this client's
  // conventions. (We build on parseRegistry rather than listMembers precisely because
  // listMembers swallows the document-level fetch/parse errors.)
  const parsed = await registryParseRegistry(registryUrl, { fetch: fetchImpl });
  const members = parsed.members.map((v) => {
    const membership: Membership = v.membership ?? { app: "" };
    ...
    return {
      ...
      trusted: status !== undefined && TRUSTED_STATUS.has(status),
      valid: v.valid,
      issues: v.issues,
    };
  });
 ...
 */
export async function resolveStorageSpecVersion(
  storageUrl: string,
  options: RegistryOptions = {},
): Promise<ResolvedStorageSpec> {
  const fetchImpl = guardedFetchFor(options);
  const result = await registryParseStorage(storageUrl, { fetch: fetchImpl });
  const storage: StorageDescription | undefined = result.storage;
  // FAIL CLOSED on the WHOLE verification, not just an absent description (roborev
  // round-2 Medium): a partial/invalid StorageDescription (e.g. valid:false but a
  // populated acceptsSpec — a doc that named a description yet failed some check) must
  // NOT advertise versions. An app must never write against an UNVERIFIABLE storage,
  // so unless the description verified clean we expose empty lists and accepts() ⇒
  // false for every version. Spec/predicate semantics are gated on result.valid.
  const acceptsSpecList = result.valid ? (storage?.acceptsSpec ?? []) : [];
  const supportsSectorList = result.valid ? (storage?.supportsSector ?? []) : [];
  // Build a Pick<StorageDescription,"acceptsSpec"> the registry helpers accept, so
  // the exact-IRI matching stays the registry's (we never re-implement it here). When
  // invalid this view is empty, so accepts()/unsupported() fail closed.
  const specView = { acceptsSpec: acceptsSpecList } as Pick<StorageDescription, "acceptsSpec">;
  return {
    id: storage?.id ?? storageUrl,
    ...(result.valid && storage?.storage !== undefined ? { storage: storage.storage } : {}),
    acceptsSpec: acceptsSpecList,
    supportsSector: supportsSectorList,
    valid: result.valid,
    issues: result.issues,
    accepts: (specVersionIri: string) => registryAcceptsSpec(specView, specVersionIri),
    unsupported: (wanted: readonly string[]) => registryUnsupportedSpecs(specView, wanted),
  };
}

/**
 * Compose the SSRF guard around the caller's `fetch`. The guard validates the
 * registry URL + every redirect hop and caps the body/time; the caller's fetch (or
 * `globalThis.fetch`) issues the actual, already-validated request.
 */
function guardedFetchFor(options: RegistryOptions): typeof globalThis.fetch {
...
// Re-export the registry's domain types a consumer of THIS module needs, so they can
// type their results (`DiscoveredMember.membership`, `ResolvedStorageSpec`) without a
// direct `@jeswr/federation-registry` import. The registry RUNTIME is inlined into our
// `dist/index.js` (so the JS is self-contained under ignore-scripts); these type
// declarations resolve because `@jeswr/federation-registry` is also a pinned git
// `dependency` whose own committed `dist/*.d.ts` provides the types (no build step).
export type {
  Membership,
