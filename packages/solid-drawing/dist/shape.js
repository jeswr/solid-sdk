...
export const DRAWING_SHAPE_PATH = fileURLToPath(new URL("../drawing.shacl.ttl", import.meta.url));
/** Filesystem path to the drawing ontology TTL. */
export const DRAWING_ONTOLOGY_PATH = fileURLToPath(new URL("../drawing.ttl", import.meta.url));
let cachedShape;
let cachedOntology;
/**
 ...
 */
export function drawingShapeTtl() {
    if (cachedShape === undefined)
        cachedShape = readFileSync(DRAWING_SHAPE_PATH, "utf8");
    return cachedShape;
}
/** The drawing ontology, as a Turtle string. Cached after the first read. */
export function drawingOntologyTtl() {
    if (cachedOntology === undefined)
        cachedOntology = readFileSync(DRAWING_ONTOLOGY_PATH, "utf8");
