...
export function replicateSolid(options) {
     ...
     */
    async function readConsistentDoc(resourceName) {
        const read = await readDoc(resourceName);
        if (read === null)
            return null;
        const docKey = String(read.doc[primaryPath]);
        if (keyToResourceName(docKey) !== resourceName)
            return null;
        return read;
    }
    /**
     ...
     */
    async function isUsableDoc(resourceName) {
        let key;
        try {
     ...
     */
    async function writeDocConditional(resourceName, doc, precondition) {
        const { body, contentType } = serializeDoc(doc);
        const opts = "create" in precondition ? { ifNoneMatch: "*" } : { ifMatch: precondition.ifMatch };
     ...
     */
    async function writeDocBestEffort(resourceName, doc) {
        const { body, contentType } = serializeDoc(doc);
    ...
    async function readMeta() {
        ...
        if (!fetched)
            return { meta: { ...EMPTY_META, index: {} }, etag: null };
        try {
     ...
     */
    async function commitMetaIndex(written) {
        ...
        for (let attempt = 0; attempt < MaxAttempts; attempt++) {
            ...
            const res = await store.putDoc(META_RESOURCE_NAME, body, DOC_CONTENT_TYPE, 
        ...
        throw new Error("[rxdb-solid] failed to commit the metadata index after repeated concurrent updates");
    }
