...
/**
 * Exhaustive SSRF tests for the guardedFetch egress chokepoint.
 *
 ...
 *  - IPv6: ::1, ::, ULA fc00::/7, link-local fe80::/10, IPv4-mapped (compressed AND expanded),
 *    6to4 2002:: embedding a private v4, NAT64 64:ff9b::.
 *  - Alternate IPv4 encodings: decimal, octal, hex, short-form — normalised before classifying.
 *  - DNS rebinding: stub returns public-then-private (multi-record refusal); pin used on connect.
 *  - Redirect → private (GET blocked, re-classified per hop), redirect cap, scheme downgrade, loop,
 *    and a POST refusing to follow ANY redirect (confused-deputy).
 *  - Oversize body (aborted), declared-Content-Length over cap, timeout, disallowed content-type,
 *    non-http scheme, userinfo, port gate, cloud-internal hostname denylist.
 *  - Happy path GET + POST against a 127.0.0.1 fixture (loopback test hook only).
 */
...
describe("isPublicAddress — IPv4 ranges", () => {
  ...
  it("allows public v4 (8.8.8.8, 1.1.1.1, just-outside /12)", () => {
  ...
  it("re-permits loopback only under allowLoopback", () => {
    ...
    expect(isPublicAddress("10.0.0.1", true)).toBe(false);
  });
...
describe("isPublicAddress — IPv6 ranges", () => {
  ...
  it("allows public v6 (2606:4700::1, public mapped, public 6to4)", () => {
    expect(isPublicAddress("2606:4700::1", false)).toBe(true);
    expect(isPublicAddress("::ffff:8.8.8.8", false)).toBe(true);
    expect(isPublicAddress("2002:0808:0808::", false)).toBe(true);
  });
  ...
  it("allows a public NAT64-prefixed address embedding a public v4", () => {
    // 64:ff9b::8.8.8.8 — well-known NAT64 prefix embedding 8.8.8.8 (public) → allowed.
    expect(isPublicAddress("64:ff9b::8.8.8.8", false)).toBe(true);
  });

  it("allows a non-well-known 64:: address that is not the NAT64 prefix", () => {
    // high hextet 0x0064 but NOT the 64:ff9b::/96 well-known prefix → not treated as NAT64.
    expect(isPublicAddress("64:1::1", false)).toBe(true);
  });

  it("classifies the EXPANDED IPv4-mapped public form as public", () => {
    // 0:0:0:0:0:ffff:0808:0808 == ::ffff:8.8.8.8 → public.
...
describe("normalizeHostForClassification — alternate IPv4 encodings", () => {
  ...
  it("returns input lowercased when URL parse fails", () => {
...
describe("assertNotSsrf — scheme / port / userinfo gates", () => {
  ...
  it("rejects http: under allowLoopback if a resolved addr is non-loopback", async () => {
    await expect(
      assertNotSsrf("http://sneaky.example/", {
        allowLoopback: true,
        enforceHttpsExceptLoopback: true,
        dnsLookup: stubDns({ address: "8.8.8.8", family: 4 }),
      })
...
describe("assertNotSsrf — DNS resolution + rebinding", () => {
  ...
  it("surfaces a DNS lookup error as SsrfError", async () => {
    await expect(
      assertNotSsrf("https://broken.example/", {
        allowLoopback: false,
...
describe("guardedFetch — happy path (loopback test hook)", () => {
  it("GET fetches an allowed RDF doc and returns body + finalUrl + content-type", async () => {
  ...
  it("POST delivers a body and returns the receipt status (allowlist skipped)", async () => {
    let seenBody = "";
    let seenCt = "";
    routes.set("/inbox-post", (req, res) => {
    ...
    const r = await guardedFetch(`${base}/inbox-post`, {
      allowLoopback: true,
      method: "POST",
    ...
    expect(seenCt).toBe("text/turtle");
    expect(seenBody).toContain("Announce");
  });
  ...
  it("sends a descriptive User-Agent", async () => {
    ...
    expect(seenUa).toMatch(/solid-agent-notify/);
  });

  it("forwards if-modified-since conditional validator", async () => {
    let seen = "";
    routes.set("/ims", (req, res) => {
      seen = String(req.headers["if-modified-since"] ?? "");
      res.writeHead(200, { "content-type": "text/turtle" });
      res.end(TTL);
    });
    await guardedFetch(`${base}/ims`, {
      allowLoopback: true,
      conditional: { lastModified: "Wed, 21 Oct 2026 07:28:00 GMT" },
    });
    expect(seen).toBe("Wed, 21 Oct 2026 07:28:00 GMT");
  });
});
...
describe("guardedFetch — content-type allowlist", () => {
  ...
  it("respects a custom allowedContentTypes set", async () => {
    routes.set("/custom", (_req, res) => {
      res.writeHead(200, { "content-type": "application/json" });
      res.end("{}");
    });
    const r = await guardedFetch(`${base}/custom`, {
      allowLoopback: true,
...
describe("guardedFetch — redirects (each GET hop re-validated)", () => {
  ...
  it("treats a 3xx WITHOUT Location as final (falls through to allowlist)", async () => {
    routes.set("/redir-noloc", (_req, res) => {
      res.writeHead(302, { "content-type": "text/turtle" });
      res.end(TTL);
    });
    const r = await guardedFetch(`${base}/redir-noloc`, {
      allowLoopback: true,
    });
...
describe("guardedFetch — POST refuses ALL redirects (confused-deputy)", () => {
  it("refuses to follow a POST redirect even to the SAME host (fails closed)", async () => {
    routes.set("/post-r1", (_req, res) => {
      res.writeHead(307, { location: `${base}/post-r2` });
      res.end();
    });
    routes.set("/post-r2", (_req, res) => {
      res.writeHead(201);
      res.end();
    });
    await expect(
      guardedFetch(`${base}/post-r1`, {
        allowLoopback: true,
        method: "POST",
        body: "x",
        skipContentTypeAllowlist: true,
  ...
  it("refuses a POST redirect to a private host (would be a token-leak bounce)", async () => {
    routes.set("/post-private", (_req, res) => {
      res.writeHead(302, { location: "https://169.254.169.254/" });
      res.end();
    });
    await expect(
      guardedFetch(`${base}/post-private`, {
        allowLoopback: true,
        method: "POST",
        body: "x",
        skipContentTypeAllowlist: true,
