...
import { type LookupAddress, assertNotSsrf, pinnedLookup } from "./ssrf.js";
      ...
      if (res.status >= 300 && res.status < 400) {
        ...
        if (location) {
          ...
          if (method === "POST") {
            ...
            throw new GuardedFetchError(
              `Refusing to follow a ${res.status} redirect on a POST: ${currentUrl} → ${location}.`
            );
          }
