...
export async function assertNotSsrf(rawUrl, opts) {
    ...
    if (opts.enforceHttpsExceptLoopback &&
        url.protocol === "http:" &&
        !opts.allowLoopback) {
        throw new SsrfError(`URL must be https: (got http: ${url.host}). HTTP is permitted only when allowLoopback=true (dev/IT).`);
    }
    ...
    if (opts.enforceHttpsExceptLoopback &&
        ...
        for (const r of resolved) {
            if (!isLoopbackAddress(r.address)) {
                throw new SsrfError(`URL refused — http: allowed only when ALL resolved addresses are loopback (got ${r.address}). Use https: in production.`);
            }
        }
    }
    for (const r of resolved) {
        if (!isPublicAddress(r.address, opts.allowLoopback)) {
            throw new SsrfError(`URL refused — ${hostname} resolves to a non-public address (${r.address}).`);
        }
    }
    return resolved[0];
}
