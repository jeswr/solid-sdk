 ...
 *
 * VENDORED VERBATIM from `solid-webid-index` `src/lib/security/addresses.ts` (itself ported from the
 * prod-solid-server `packages/guarded-fetch` copy, which came from the resource server's exhaustively
 * tested WebID resolver). The SSRF primitives are vendored unchanged so the proven test suite carries
 * over; only the module doc comment differs from the source.
 *
 ...
 */
export function isPublicAddress(address, allowLoopback) {
