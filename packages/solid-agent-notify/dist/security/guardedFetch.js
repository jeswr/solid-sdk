...
import { FETCH_HOSTNAME_DENYLIST, FETCH_TIMEOUT_MS, FETCH_USER_AGENT, MAX_BYTES_PROFILE, MAX_REDIRECTS, RDF_ACCEPT, RDF_CONTENT_TYPES, } from "../config.js";
...
export async function guardedFetch(rawUrl, opts = {}) {
    ...
    if (method === "POST" && status >= 300 && status < 400) {
        ...
        if (location) {
            ...
            throw new GuardedFetchError(`Refusing to follow a ${status} redirect on a POST: ${finalUrl} → ${location}.`);
        }
    }
 ...
 */
export function assertSchemeAndPort(url, allowLoopback, prevWasHttps = false) {
 ...
 */
export function classifyGuardError(error, rawUrl, method) {
    if (error instanceof SsrfError) {
        ...
        if (method === "POST" && isRedirectMgmt) {
            return new GuardedFetchError(`Refusing to follow a redirect on a POST: ${rawUrl} (${msg}).`, { cause: error });
        }
        if (isTimeout) {
