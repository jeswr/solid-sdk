...
describe("custom-elements.json — CEM accuracy", () => {
  ...
  it("each per-class resolver entry's (class, mode) is advertised by exactly one element's @solid block", () => {
    // Phase-2: the SAME class is advertised by TWO elements — a view list + an edit
    // form — so the manifest↔map check must key on (class, MODE), not class alone.
    // The composite key joins class + mode with a single SPACE delimiter: an IRI cannot
    // contain whitespace (RFC 3987) and a mode is a single word (view/edit), so the
    // space can never collide across the class↔mode boundary. (The delimiter must NOT be
    // a raw NUL byte — that turns this `.ts` source binary and breaks TS/lint/diff.)
    const tagByClassMode = new Map<string, string>();
    for (const d of classDecls()) {
      const cls = d.solid?.class;
      const mode = d.solid?.mode;
      if (cls && mode && d.tagName) tagByClassMode.set(`${cls} ${mode}`, d.tagName);
    }
    // A resolver class that an element ADVERTISES via @solid-class must route to that
    // same element's tag FOR THAT MODE. The map ALSO carries a few ALIAS entries that
    // no single element advertises (a Lit element's @solid-class JSDoc tag holds ONE
    // value, but an element legitimately renders SEVERAL classes): vcard:AddressBook
    ...
    for (const entry of ResolverEntries) {
      ...
      expect(manifestTag, `resolver class ${entry.targetClass} (${entry.mode})`).toBe(
        entry.tagName,
      );
    }
  });

  it("every per-class EDIT form carries @solid-mode edit + its bound class", () => {
    const byName = new Map(classDecls().map((d) => [d.name, d]));
    const expectations: Record<string, string> = {
      JeswrTaskForm: "http://www.w3.org/2005/01/wf/flow#Task",
      JeswrContactForm: "http://www.w3.org/2006/vcard/ns#Individual",
      JeswrBookmarkForm: "https://w3id.org/jeswr/bookmark#Bookmark",
    };
    for (const [name, cls] of Object.entries(expectations)) {
      const d = byName.get(name);
      expect(d, `${name} present in manifest`).toBeDefined();
      expect(d?.solid?.mode, `${name} is @solid-mode edit`).toBe("edit");
      expect(d?.solid?.class, `${name} binds ${cls}`).toBe(cls);
      expect(d?.solid?.cardinality).toBe("one");
    }
    // The generic editable <jeswr-shacl-form> base binds no class (no @solid block).
    expect(byName.get("JeswrShaclForm")?.solid?.class).toBeUndefined();
  });
});
