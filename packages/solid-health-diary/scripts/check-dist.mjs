...
// fileURLToPath (not `.pathname`) so a checkout path with spaces / escaped chars
// decodes correctly on the filesystem.
...
const tmp = mkdtempSync(join(tmpdir(), "shd-dist-"));
try {
  ...
  if (extra.length) errors.push(`dist/ has STALE files not produced by a fresh build: ${extra.join(", ")}`);
