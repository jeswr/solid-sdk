...
export const DIET_SHACL_PATH: string = fileURLToPath(
  new URL("../shapes/diet.shacl.ttl", import.meta.url),
);

/** Filesystem path to the vendored vocabulary (`shapes/diet.vocab.ttl`). */
export const DIET_VOCAB_PATH: string = fileURLToPath(
  new URL("../shapes/diet.vocab.ttl", import.meta.url),
);

let cachedShacl: string | undefined;
let cachedVocab: string | undefined;
 ...
 */
export function dietShaclTtl(): string {
  if (cachedShacl === undefined) cachedShacl = readFileSync(DIET_SHACL_PATH, "utf8");
  return cachedShacl;
}
 ...
 */
export function dietVocabTtl(): string {
  if (cachedVocab === undefined) cachedVocab = readFileSync(DIET_VOCAB_PATH, "utf8");
  return cachedVocab;
}
