...
import { DIET_TRIGGER_CLASS, diet, isTriggerSlug, rdf, SKOS_CONCEPT, skos, triggerIri, triggerSlugFromIri, } from "./vocab.js";
 ...
 */
export function toXsdDecimalLexical(value) {
 ...
 */
function decimalFrom(value, factory) {
 ...
 */
export function isValidLagProfile(p) {
 ...
 */
export const EVIDENCE_PRIOR_LAG = {
 ...
 */
export function defaultTriggerClass(slug) {
...
export function triggerClassSubject(slug) {
 ...
 */
export function buildTriggerClass(data) {
    ...
    if (!isTriggerSlug(data.slug)) {
        throw new Error(`buildTriggerClass: slug must be a known TriggerClass — got ${JSON.stringify(data.slug)}.`);
    }
    ...
    if (!isValidLagProfile(data)) {
        throw new Error(`buildTriggerClass: lag profile must be finite, non-negative and ordered ` +
...
export function serializeTriggerClass(data) {
 ...
 * concept per slug.
 */
export function buildTriggerScheme(overrides = {}) {
    const store = new Store();
    for (const slug of Object.keys(EVIDENCE_PRIOR_LAG)) {
        ...
        if (!isValidLagProfile(merged)) {
            throw new Error(`buildTriggerScheme: the override for "${slug}" yields an invalid lag profile ` +
