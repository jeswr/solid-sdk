...
export function canonicalToLongChat(msg, subject) {
 ...
 */
export async function parseAs2(baseIri, body, contentType = "text/turtle", subject = as2MessageSubject(baseIri)) {
 ...
 * @param subject - the message subject IRI; defaults to `${baseIri}#it`.
 */
export async function parseLongChat(baseIri, body, contentType = "text/turtle", subject = longChatMessageSubject(baseIri)) {
    const dataset = await parseToDataset(baseIri, body, contentType);
    return parseLongChatMessage(subject, dataset);
}
/** Serialise the canonical model as an AS2.0 Turtle document rooted at `subject`. */
export function serializeAs2(msg, subject) {
...
export function serializeLongChat(msg, subject) {
 ...
 */
async function parseToDataset(baseIri, body, contentType) {
 ...
 */
export async function roundTripAs2ToLongChat(msg, subject, opts = {}) {
    const as2Turtle = await serializeAs2(msg, subject);
    const fromAs2 = await parseAs2(subject, as2Turtle, "text/turtle", subject);
    if (!fromAs2)
        throw new Error("round-trip: AS2.0 message did not parse back");
    ...
    if (!fromLongChat)
        throw new Error("round-trip: LongChat message did not parse back");
    if (opts.lossy)
        return fromLongChat;
    ...
    const result = { ...fromLongChat };
    if (fromAs2.room !== undefined)
        result.room = fromAs2.room;
    if (fromAs2.mediaType !== undefined)
        result.mediaType = fromAs2.mediaType;
    return result;
}
