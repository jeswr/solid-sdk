...
// then fails in CI / Vercel / any fresh environment without one. This guard
// stops a stray `npm install` from quietly re-breaking the lockfile.
//
// It FAILS (exit 1) if any committed package-lock.json (root or nested) contains
// an SSH git transport. stdlib-only Node ESM, no dependency, so it is safe under
// ignore-scripts=true.
//
...
// or the scp-like `ssh://git@github`; match both.
