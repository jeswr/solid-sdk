...
export async function importRoom(options) {
    const { homeserverUrl, accessToken, roomId, writeFetch, webIdFor, writeAcl = true, ownerWebId, } = options;
    ...
    for (const eventId of [...states.keys()].sort()) {
        ...
        if (base === undefined)
            continue; // nothing to write (shouldn't happen)
 ...
 */
function foldResult(states, result) {
    switch (result.kind) {
        case "message": {
