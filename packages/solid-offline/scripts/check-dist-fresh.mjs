 ...
 *    (`status-<hash>.d.ts`) whose filename carries a content hash. We compare the
 *    full SET of emitted `.js`/`.d.ts` artifacts by relative path, so a chunk that
 *    is added, removed, or whose hash changes (because the shared source changed)
 *    surfaces as drift — even though tsup's hash is deterministic for given input.
 *  - It writes `*.map` sourcemaps whose byte content varies with absolute build
 *    paths (the scratch outDir differs from the committed build's cwd). Sourcemaps
 *    are not load-bearing for a consumer importing the package, so they are
 *    deliberately ignored; only code (`.js`) and types (`.d.ts`) are compared.
 *
 ...
 * skipping `*.map` sourcemaps (non-deterministic, not load-bearing).
 */
...
try {
  ...
  // Rebuild into a scratch outDir using the repo's tsup config (tsup.config.ts).
  execFileSync("npx", ["tsup", "--out-dir", freshDist], {
    cwd: root,
