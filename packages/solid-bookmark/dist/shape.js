...
export const BOOKMARK_ONTOLOGY_PATH = fileURLToPath(new URL("../bookmark.ttl", import.meta.url));
/** Filesystem path to the canonical bookmark SHACL shape (`bookmark.shacl.ttl`). */
export const BOOKMARK_SHAPE_PATH = fileURLToPath(new URL("../bookmark.shacl.ttl", import.meta.url));
let cachedOntology;
let cachedShape;
/**
 ...
 */
export function bookmarkOntologyTtl() {
 ...
 */
export function bookmarkShapeTtl() {
    if (cachedShape === undefined)
        cachedShape = readFileSync(BOOKMARK_SHAPE_PATH, "utf8");
    return cachedShape;
}
