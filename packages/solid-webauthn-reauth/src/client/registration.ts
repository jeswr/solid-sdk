...
import type { RegistrationOptions, RegistrationResponseJSON } from "../protocol/index.js";
...
function requireAbsoluteUrl(value: string | URL, field: string): string {
  try {
...
export async function registerPasskey(
  ...
  const webId =
    options.webId !== undefined ? requireAbsoluteUrl(options.webId, "webId") : undefined;
  ...
  if (!optionsResponse.ok) {
    throw new Error(
      `registerPasskey: register-options request failed: ${optionsResponse.status} ${optionsResponse.statusText}`,
    );
  }
  const rawOptions = (await optionsResponse.json()) as unknown;
  if (typeof rawOptions !== "object" || rawOptions === null) {
    throw new Error("registerPasskey: register-options response was not an object");
  }
  const optionsJSON = rawOptions as RegistrationOptions;
  ...
  const creationOptions: RegistrationOptions = requireResidentKey
    ? {
        ...optionsJSON,
  ...
  if (!registerResponse.ok) {
    throw new Error(
      `registerPasskey: register request failed: ${registerResponse.status} ${registerResponse.statusText}`,
    );
  }
