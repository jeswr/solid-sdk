...
// then fails in CI / Vercel / any fresh environment without one. This guard fails
// if any committed package-lock.json contains an SSH git transport. This package
// has no `@jeswr` github: deps today, so it is a no-op guard kept for suite
// consistency (and to stay safe if one is added later).
//
...
// hosted-git-info rewrites to `git+ssh://git@github` or the scp-like
// `ssh://git@github`; match both.
...
if (bad > 0) {
  console.error(
    `\ncheck:lockfile-transport — ${bad} lockfile(s) contain an SSH git transport.\n` +
      "  Rewrite each `git+ssh://git@github.com/...` to `git+https://github.com/...`.",
  );
  process.exit(1);
}
