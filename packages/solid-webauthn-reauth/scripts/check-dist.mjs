...
// github:jeswr/solid-webauthn-reauth#main` and import it under
// `ignore-scripts=true` with NO build step. That only stays honest if the
// committed artifact matches the source: this gate rebuilds (BOTH the ESM and the
// CJS-protocol builds) into a sibling TEMP dir and byte-compares every emitted
// file against the committed `dist/`. A mismatch (or a missing/extra file) fails
// — the fix is `npm run build` + commit.
//
// The temp dir is a SIBLING of dist/ at the repo root (same directory depth) so
// source-map relative `sources` paths (map → src) match the committed build
// exactly — a temp dir elsewhere on disk shifts those relative paths and makes
// the .map byte-comparison spuriously fail.
//
...
// 2. Build fresh into the sibling temp dir (cleaned first / on exit).
rmSync(tmp, { recursive: true, force: true });
try {
  execFileSync(
    process.execPath,
    [tsc, "-p", join(root, "tsconfig.esm.json"), "--outDir", join(tmp, "esm")],
    {
      stdio: "inherit",
      cwd: root,
    },
  ...
  execFileSync(
    process.execPath,
    [tsc, "-p", join(root, "tsconfig.cjs.json"), "--outDir", join(tmp, "cjs")],
    {
      stdio: "inherit",
      cwd: root,
    },
  );
  mkdirSync(join(tmp, "cjs"), { recursive: true });
  writeFileSync(
    join(tmp, "cjs", "package.json"),
