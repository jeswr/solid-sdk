 ...
 */
function escapeIri(v: string): string {
...
const blankNodeLabel = (term: TermWrapper): string => term.value;

// --- Plain data interfaces (the shape an app's UI works with) ---
...
export class ContactBook extends TermWrapper {
  ...
  get title(): string | undefined {
    return (
      OptionalFrom.subjectPredicate(this, dc("title"), LiteralAs.string) ??
   ...
   */
  private defaultIndex(name: "people.ttl" | "groups.ttl"): string | undefined {
...
function addStructuredValue(
  ...
  SetFrom.subjectPredicate(parent, predicate, blankNodeLabel, BlankNodeFrom.string).add(
    bnode.value,
  );
 ...
 */
function readStructuredValues(parent: TermWrapper, predicate: string): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  const p = parent.factory.namedNode(predicate);
  for (const q of parent.dataset.match(parent as unknown as Term, p)) {
    const obj = q.object;
...
export class Contact extends TermWrapper {
  ...
  setEmails(emails: string[]): void {
    ...
    for (const raw of emails) {
      ...
      if (isMailto(iri)) addStructuredValue(this, VCARD_HAS_EMAIL, VCARD_HOME, escapeIri(iri));
    }
  }
  ...
  setPhones(phones: string[]): void {
    ...
    for (const raw of phones) {
      ...
      if (isTel(iri)) addStructuredValue(this, VCARD_HAS_TELEPHONE, VCARD_CELL, escapeIri(iri));
    }
  }
  ...
  setWebId(webId: string | undefined): void {
    ...
    if (isHttpIri(webId)) addStructuredValue(this, VCARD_URL, VCARD_WEB_ID, webId);
  }
   ...
   */
  private clearStructured(predicate: string): void {
    const p = this.factory.namedNode(predicate);
    const edges = [...this.dataset.match(this as unknown as Term, p)];
    for (const q of edges) {
      if (q.object.termType === "BlankNode") {
        for (const inner of [...this.dataset.match(q.object)]) this.dataset.delete(inner);
      }
...
export class ContactGroup extends TermWrapper {
  ...
  setMembers(members: string[]): void {
    ...
    for (const m of [...live]) live.delete(m);
    for (const m of members) if (isHttpIri(m)) live.add(m);
  }
}
...
export function parseAddressBook(
  bookDocUrl: string,
  dataset: DatasetCore,
): AddressBookData | undefined {
  const book = new ContactBook(addressBookSubject(bookDocUrl), dataset, DataFactory);
  if (!book.isAddressBook) return undefined;
  const data: AddressBookData = { title: book.title ?? "" };
  if (book.nameEmailIndex !== undefined) data.nameEmailIndex = book.nameEmailIndex;
  if (book.groupIndex !== undefined) data.groupIndex = book.groupIndex;
  if (book.owner !== undefined) data.owner = book.owner;
  return data;
}
...
export async function serializeAddressBook(
  bookDocUrl: string,
 ...
 */
export async function parseAddressBookTtl(
  bookDocUrl: string,
  body: string,
  contentType: string | null = "text/turtle",
): Promise<AddressBookData | undefined> {
...
export function parsePerson(personDocUrl: string, dataset: DatasetCore): ContactData | undefined {
  ...
  if (!person.isIndividual) return undefined;
  const emails = person.emails;
  const phones = person.phones;
  const data: ContactData = { name: person.name ?? "" };
  if (person.inAddressBook !== undefined) data.inAddressBook = person.inAddressBook;
...
export async function serializePerson(personDocUrl: string, data: ContactData): Promise<string> {
 ...
 */
export async function parsePersonTtl(
  personDocUrl: string,
  body: string,
  contentType: string | null = "text/turtle",
...
export function parseGroup(
  groupDocUrl: string,
  dataset: DatasetCore,
): ContactGroupData | undefined {
  const group = new ContactGroup(groupSubject(groupDocUrl), dataset, DataFactory);
  if (!group.isGroup) return undefined;
  const members = [...group.members];
  const data: ContactGroupData = { name: group.name ?? "" };
  if (group.inAddressBook !== undefined) data.inAddressBook = group.inAddressBook;
...
export async function serializeGroup(groupDocUrl: string, data: ContactGroupData): Promise<string> {
 ...
 */
export async function parseGroupTtl(
  groupDocUrl: string,
  body: string,
  contentType: string | null = "text/turtle",
): Promise<ContactGroupData | undefined> {
  const resolvedContentType = contentType ?? "text/turtle";
  const { parseRdf } = await import("@jeswr/fetch-rdf");
...
export function buildPeopleIndex(
  bookSubjectIri: string,
  entries: { person: string; name: string }[],
): Store {
  const store = new Store();
...
export function serializePeopleIndex(
  bookSubjectIri: string,
  entries: { person: string; name: string }[],
): Promise<string> {
 ...
 */
export function buildGroupsIndex(
  bookSubjectIri: string,
  entries: { group: string; name: string }[],
): Store {
  const store = new Store();
  if (!isHttpIri(bookSubjectIri)) return store;
  const book = new ContactBook(bookSubjectIri, store, DataFactory);
  for (const { group, name } of entries) {
    if (!isHttpIri(group)) continue;
...
export function serializeGroupsIndex(
  bookSubjectIri: string,
  entries: { group: string; name: string }[],
): Promise<string> {
