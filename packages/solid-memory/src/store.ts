...
export class MemoryStore {
  ...
  private async request(url: string, init: RequestInit): Promise<Response> {
    ...
    if (res.type === "opaqueredirect" || (res.status >= 300 && res.status < 400)) {
      ...
      throw new Error(
        `[solid-memory] ${init.method ?? "GET"} ${url} responded with a redirect (${shown}) — ` +
  ...
  async update(
    url: string,
    data: MemoryData,
    opts?: { ifMatch?: string; assumeNotForgotten?: boolean },
  ): Promise<{ etag?: string }> {
    assertWithinBase(this.container, url);
    ...
    if (created === undefined || !tombstoneResolved) {
      let existing: Awaited<ReturnType<MemoryStore["get"]>>;
      try {
        ...
        if (!tombstoneResolved) {
          throw new Error(
            `[solid-memory] update ${url} refused: could not read the existing resource to ` +
      ...
      if (created === undefined) created = existing?.data.created;
    ...
    const withModified: MemoryData = { ...data, created, invalidatedAt, modified: new Date() };
  ...
  async forget(
    url: string,
    opts?: { ifMatch?: string; at?: Date },
  ): Promise<{ etag?: string; invalidatedAt: Date }> {
    assertWithinBase(this.container, url);
    const existing = await this.get(url);
    if (!existing) {
      throw new Error(
        `[solid-memory] forget ${url} failed: no mem:MemoryItem to forget (missing or not a memory) — use delete() for a hard remove`,
      );
    }
   ...
   *   `mem:MemoryItem` / unreadable, or the PUT is rejected (incl. a 412).
   */
  async unforget(url: string, opts?: { ifMatch?: string }): Promise<{ etag?: string }> {
    assertWithinBase(this.container, url);
    const existing = await this.get(url);
    if (!existing) {
      throw new Error(
        `[solid-memory] unforget ${url} failed: no mem:MemoryItem to un-forget (missing or not a memory)`,
      );
    }
    ...
    const cleared: MemoryData = { ...existing.data, modified: new Date() };
