...
export const mem = (local: string): string => `${MEM}${local}`;
...
export const as = (local: string): string => `${AS}${local}`;
/** Build a `pc:` term IRI. */
export const pc = (local: string): string => `${PC}${local}`;
...
export const MEMORY_CLASS = mem("MemoryItem");

/** `mem:embeddingRef` — optional pointer to a sidecar embedding resource (M2 vector seam). */
export const MEM_EMBEDDING_REF = mem("embeddingRef");
