...
import { decideSilentRestore, hasPersisted, IndexedDbSessionStore, indexedDbAvailable, RememberedAccount, restoreSession, shouldDropRememberedPointer, webIdsEqual, } from "@jeswr/solid-session-restore";
 ...
 */
const MODULE_PRISTINE_FETCH = typeof globalThis !== "undefined" && typeof globalThis.fetch === "function"
...
export function isUseDpopNonceChallenge(response) {
    ...
    for (const challenge of parseWwwAuthenticate(header)) {
        ...
        if (error === "use_dpop_nonce")
            ...
            return false; // a DPoP challenge with a DIFFERENT error → ambiguous → force refresh
    }
    return sawNonce;
}
/**
...
class RecentAccountsList {
        ...
        try {
            ...
            for (const a of parsed) {
                if (typeof a !== "object" || a === null)
                    continue;
                const rec = a;
                if (typeof rec.webId !== "string" || rec.webId.length === 0)
                    continue;
                out.push({
                    webId: rec.webId,
    ...
    remember(account) {
        try {
            ...
            globalThis.localStorage?.setItem(this.#key, JSON.stringify([account, ...rest].slice(0, MAX_RECENT_ACCOUNTS)));
        }
        catch {
 ...
 */
export function createReactiveAuthController(options) {
    return new ReactiveAuthController(options);
}
class ReactiveAuthController {
    /**
    ...
    constructor(options) {
        ...
        this.#publicFetch =
            options.publicFetch ??
                ...
                (() => Promise.reject(new Error("No pristine fetch available in this environment")));
        ...
        this.#recentAccounts = new RecentAccountsList(options.recentAccountsKey ??
    ...
    async restore() {
        if (this.#restoreInFlightPromise)
            return this.#restoreInFlightPromise;
        ...
        try {
            ...
            if (this.#restoreInFlightPromise === run)
    ...
    async #restoreIssuer(issuer, expectedWebId) {
        ...
        const restored = await this.#withGrantAbort((signal) => restoreSession({
            store: guarded.store,
            issuer,
        ...
        this.#pinRestoredSession(generation, issuer, restored.webId, restored.accessToken, restored.dpopKey, restored.expiresAt);
        return { webId: restored.webId };
    }
     ...
     */
    #ensureProvider() {
        if (!this.#provider) {
            this.#provider = new PersistingDPoPTokenProvider(() => this.#session, (session) => this.#refreshSession(session));
        }
        if (this.#opts.patchGlobalFetch && typeof globalThis !== "undefined") {
            ...
            if (!this.#globalFetchWrapper) {
                this.#globalFetchWrapper = ((input, init) => this.#authenticatedFetchOver(this.#publicFetch, input, init));
            }
    ...
    async login(webId) {
        ...
        try {
            ...
            const { wrote: credentialWritten, durable: credentialRestorable } = await this.#persist(session, generation);
     ...
     */
    #abortActiveGrants() {
        for (const g of this.#activeGrants)
            g.abort.abort();
    }
    /**
    ...
    async logout() {
        ...
        if (issuer) {
            ...
            if (!deleted) {
                throw new Error("Logged out locally, but the persisted credential could not be deleted from durable " +
    ...
    async #resolveIssuer(webId) {
        ...
        if (!advertised) {
            throw new Error(`chooseIssuer returned an issuer (${chosen}) that the WebID profile does not advertise.`);
        }
        return chosenUrl;
    }
    #httpOptions(issuer) {
    ...
    async #resolveClient(authorizationServer, http) {
        ...
        const registrationResponse = await oauth.dynamicClientRegistrationRequest(authorizationServer, {
            redirect_uris: [this.#opts.callbackUri],
    ...
    async #authenticate(generation, issuer, expectedWebId) {
        ...
        const runLeg = async (withPromptNone) => {
            ...
            const exchange = async () => {
                const tokenResponse = await oauth.authorizationCodeGrantRequest(authorizationServer, client, oauth.None(), params, this.#opts.callbackUri, codeVerifier, // PKCE always on (S256) — never oauth.nopkce for a public client
            ...
            if (tokenResult.token_type.toLowerCase() !== "dpop") {
                throw new Error(`Expected a DPoP-bound token but the identity provider returned token_type="${tokenResult.token_type}".`);
            }
        ...
        try {
            ...
            if (!webIdsEqual(result.webId, expectedWebId))
        ...
        if (!webIdsEqual(webId, expectedWebId)) {
            throw new Error(`Signed in as a different WebID than requested (asked for ${expectedWebId}, got ${webId}).`);
        }
        return {
            generation,
            issuer,
    ...
    #guardedStore(generation) {
        ...
        const guard = (op, onPut) => {
            const run = this.#persistChain.then(async () => {
                if (generation !== this.#generation)
                    return undefined; // superseded → skip
                const result = await op();
                if (onPut)
                    rotationPersisted = true; // this put RAN to completion (not skipped/threw)
                return result;
            });
            this.#persistChain = run.then(() => undefined, () => undefined);
            return run;
 ...
 * hand-rolled crypto.
 */
class PersistingDPoPTokenProvider {
    #getSession;
    #refresh;
    /**
    ...
    rememberNonce(response, request) {
        ...
        if (!this.#allowed(session, request))
            return false;
        let origin;
        try {
    ...
    async upgrade(request, forceRefresh = false) {
        ...
        if (!this.#allowed(session, request) || !session)
            return request;
        ...
        if (shouldRefresh(session.expiresAt, forceRefresh)) {
            ...
            if (current !== session || !this.#allowed(current, request))
                return request;
            session = current;
        }
        const headers = new Headers(request.headers);
        headers.set("DPoP", await DPoP.generateProof(session.dpopKey, 
        ...
        htuOf(request.url), request.method, 
        ...
        this.#nonceFor(request.url), session.accessToken));
