 ...
 */
const PANEL_PRISTINE_FETCH: typeof fetch | undefined =
...
type Phase =
  // Deciding silent restore on load (controller present + auto-restore on).
  | "restoring"
  // No session; show the login prompt (WebID input + recent accounts).
  | "idle"
  // Interactive login in flight (the popup / dialog is open).
  | "authenticating"
  // Logged in; show the signed-in summary + sign-out.
  | "authenticated";
...
export class JeswrLoginPanel extends LitElement {
   ...
   */
  readonly #nativeFetch: typeof fetch | undefined = PANEL_PRISTINE_FETCH;
  ...
  #publicFetchFallback(): typeof fetch {
    if (this.#nativeFetch) return this.#nativeFetch;
    ...
    return (() =>
      Promise.reject(new Error("No fetch available in this environment"))) as typeof fetch;
  }
  ...
  protected override willUpdate(changed: Map<PropertyKey, unknown>): void {
    ...
    if (!changed.has("controller")) return;
    const previous = changed.get("controller") as LoginController | undefined;
   ...
   */
  #maybeStartRestore(): void {
    if (this.#restoreStarted) return;
    const controller = this.controller;
    if (!controller) return; // inert until wired
    ...
    void controller
      .restore()
      .then((result) => {
        if (generation !== this.#generation) return; // superseded (disconnect/relogin)
  ...
  #doLogin = async (webId?: string): Promise<void> => {
    const controller = this.controller;
    if (!controller) return;
    ...
    try {
      ...
      this._phase = "authenticated";
      this.dispatchEvent(
        new CustomEvent<LoginDetail>("login", {
  ...
  #emitSessionChange(): void {
    ...
    this.dispatchEvent(
      new CustomEvent<SessionChangeDetail>("session-change", {
  ...
  private renderAccount(account: RecentLoginAccount) {
  ...
  private renderPrompt() {
    ...
    const showInput =
      this._showInput || accounts.length === 0 || this._webIdInput.trim().length > 0;
    return html`
                ...
                <label class="field">
                  ...
                  <input
                    ...
                    @input=${(ev: Event) => {
                      ...
                      this._webIdInput = (ev.target as HTMLInputElement).value;
                    }}
                  />
                </label>
                <button
