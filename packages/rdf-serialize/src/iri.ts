 ...
 * set.
 */
const FORBIDDEN_SYMBOL_CODES: ReadonlySet<number> = new Set([
  0x3c, // <
  0x3e, // >
  0x22, // "
  0x7b, // {
...
function containsForbidden(value: string): boolean {
  for (const ch of value) {
    if (isForbidden(ch.codePointAt(0) as number)) {
      return true;
    }
  }
  return false;
...
function hasEdgeControlOrSpace(value: string): boolean {
  return (
    value.length > 0 && (value.charCodeAt(0) <= 0x20 || value.charCodeAt(value.length - 1) <= 0x20)
  );
}
