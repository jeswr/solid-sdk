 ...
 * no inlining. This script therefore rebuilds with `tsc -p tsconfig.build.json`
 * into a scratch outDir and diffs the emitted JavaScript + declarations against
 * the version of `dist/` at git HEAD.
 *
...
try {
  ...
  // Rebuild into a scratch outDir with the SAME plain-tsc build pipeline that
  // `npm run build` uses (`tsc -p tsconfig.build.json`), redirecting outDir.
  execFileSync(
    "node",
    [
