 ...
 *
 * `@jeswr/agent-authz-verifier` composes two OFF-NPM git deps —
 * `@jeswr/solid-vc` and `@jeswr/solid-odrl` (which itself pulls the off-npm
 * `@jeswr/rdf-serialize`). A consumer running
 * `npm install github:jeswr/agent-authz-verifier#main` under the suite's
 * `ignore-scripts=true` invariant will NOT run our `prepare`. Both git deps DO
 * commit a self-contained `dist/`, but to make OUR install artifact robust
 * regardless of how those deps were resolved we INLINE every `@jeswr/*` package
 * into `dist/index.js`. Everything npm-published (n3, jose, rdf-canonize,
 * multiformats, @rdfjs/*, content-type, jsonld-streaming-parser) stays
 * EXTERNAL — resolved from npm by the consumer (single shared copy + normal
 * dedupe/audit).
 *
 * Externalisation contract (the load-bearing part):
 *   - INLINED  (bundled into dist): the `@jeswr/*` git deps only —
 *       `@jeswr/solid-vc`, `@jeswr/solid-odrl`, and their off-npm transitives
 *       (`@jeswr/rdf-serialize`, `@jeswr/guarded-fetch`, `@jeswr/fetch-rdf`).
 ...
 *       plus the known transitive externals the inlined RDF stack pulls in.
 *
