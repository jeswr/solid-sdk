...
// @jeswr/app-shell <FeedbackButton/> renders with pod-health's OWN repo
// (`jeswr/pod-health`) and app name, and opening it surfaces the themed feedback
...
// the load-bearing, app-specific contract here is "files against jeswr/pod-health".
//
...
describe("header FeedbackButton (pod-health adoption)", () => {
  it("renders the Feedback trigger with repo=jeswr/pod-health wiring", () => {
    render(
      <FeedbackButton
  ...
  it("opens the feedback dialog targeting the app's own repo", async () => {
    ...
    // GitHub URL — built from repo="jeswr/pod-health" — is unit-tested in
    // app-shell's own buildIssueUrl suite.)
    const dialog = await screen.findByRole("dialog");
    expect(dialog).toBeInTheDocument();
    expect(dialog).toHaveTextContent(/Pod Health/i);
  });
});
