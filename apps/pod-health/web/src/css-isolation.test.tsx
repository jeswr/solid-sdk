...
// rollout #67/#68/#70).
//
// pod-health RELAXED its CSS-leak workaround back to a BARE global `button {}` filled
// rule + a direct re-alias of the host's `--accent` / `--accent-ink` onto the
// app-shell tokens (dropping the earlier `.login-form button` hand-scoping + the
// `--host-accent` / `--muted-text` renaming), because @jeswr/app-shell #80 made the
// shell SELF-ISOLATING. This test pins WHY that relaxation is safe so a future
...
describe("app-shell CSS isolation survives pod-health's bare button {} (#80)", () => {
  it("every header control App renders is isolation-tagged", () => {
    ...
    const { container } = render(
      ...
      <ThemeProvider>
        <FeedbackButton repo="jeswr/pod-health" appName="Pod Health" appVersion="testsha" />
        <ThemeToggle />
        <AccountMenu
  ...
  it("every control in the portaled feedback dialog is isolation-tagged", async () => {
    render(<FeedbackButton repo="jeswr/pod-health" appName="Pod Health" />);
