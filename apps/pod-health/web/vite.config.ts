...
//
// Vite config for the Pod Health static host shell. It bundles the LOCAL
// `pod-health/ui` React source directly (no pre-built dist/ui needed — Vite/esbuild
// compile the TS source on the fly), wraps it in a standalone Solid login, and
// emits a fully static `dist/` (index.html + clientid.jsonld + callback.html +
// hashed assets) servable by any file server (Caddy file_server).
...
// Resolve the `pod-health` library to its LOCAL TypeScript SOURCE, so Vite
...
const podHealthSrc = (rel: string) => fileURLToPath(new URL(`../src/${rel}`, import.meta.url));
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      { find: "pod-health/ui", replacement: podHealthSrc("ui/index.ts") },
      { find: "pod-health", replacement: podHealthSrc("index.ts") },
    ...
    // (git+https pinned sha, committed dist/) — but a package can still
  ...
  // @jeswr/app-shell package is a normal node_modules install (published git+https
  // dep) so it is inside the default fs allow-list; the parent-dir allowance
  // below remains for the out-of-root pod-health source.
