...
//
// Vite config for the Pod Photos static host shell. It bundles the LOCAL
// `@jeswr/pod-photos/ui` React source directly (no pre-built dist/ui needed —
...
// Resolve the @jeswr/pod-photos library to its LOCAL TypeScript SOURCE, so Vite
...
const podPhotosSrc = (rel: string) => fileURLToPath(new URL(`../src/${rel}`, import.meta.url));
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      { find: "@jeswr/pod-photos/ui", replacement: podPhotosSrc("ui/index.ts") },
      { find: "@jeswr/pod-photos", replacement: podPhotosSrc("index.ts") },
