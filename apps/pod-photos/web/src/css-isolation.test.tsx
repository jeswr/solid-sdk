...
//
// pod-photos RELAXED its hand-scoped CSS-leak workaround back to a BARE global
// `button {}` filled rule + a direct re-alias of the host's `--accent` onto the
// app-shell token, because @jeswr/app-shell #80 made the shell SELF-ISOLATING.
// (pod-photos keeps `--muted` as the shell's SURFACE token — it uses it directly
// for the photo thumbnails — so only `--accent` was re-unified; see styles.css.)
// This test pins WHY the relaxation is safe so a future app-shell bump that
// regressed the isolation would be caught here, not in production:
//
...
describe("app-shell CSS isolation survives pod-photos's bare button {} (#80)", () => {
