...
// storage root and NEVER a count that scales with the gallery size.
//
// REGRESSION SURFACE — pod-photos' listGallery loads a gallery screen by ONE GET on the
// container AND THEN reading EACH photo document individually (the N+1 walk in
// src/ui/gallery.ts: `for (const photoUrl of photoUrls) { … freshRdf(photoUrl) }`), so
// a gallery of N photos performs N+1 pod reads on load. Under the old reactive manager
// those would be N+1 wasted 401s; the proactive patch keeps the 401 count flat (≤1 per
// storage root).
//
...
//   • Wait for the photo gallery to render its N photo tiles (login resolves the photos
//     container + loads it) and assert the resource-server 401 count is bounded and does
//     NOT scale with the photo count.
...
/** Drive the WebID-first popup login. Returns once the photo gallery is on screen.
 *
...
async function login(page: Page, seeded: Seeded): Promise<boolean> {
  ...
  // Back on the app: the photo gallery section appears once logged in (after the host
  // resolves the photos container under the pod root).
  await expect(page.locator(".pod-photos-gallery")).toBeVisible({ timeout: 60_000 });
  return true;
}

test.describe("401-budget — the proactive auth-fetch eliminates the 401-dance", () => {
  test("a session loads the photo gallery paying ≤1 resource-server 401 per storage root", async ({
    page,
  }) => {
    ...
    // The gallery loads its photos container on login. listGallery does ONE container GET
    // AND THEN reads each photo document individually — the N+1 regression surface. Wait
    // for the N photo tiles to render (one <li.pod-photos-tile> per photo), proving we
    // actually performed the per-photo reads.
    await expect
      .poll(async () => page.locator(".pod-photos-grid li.pod-photos-tile").count(), {
        timeout: 30_000,
      })
      .toBeGreaterThanOrEqual(seeded.photoCount);
    ...
    // (c) REGRESSION GUARD: the 401 count does NOT scale with the photo count. Under the
    //     old reactive manager every read (the listing + each per-photo GET) paid its own
    //     401, so a gallery of N photos would push `total` toward N+1. The proactive patch
    //     keeps it flat — strictly below the photo count.
    expect(
      total,
      `401 count (${total}) must not scale with photo count (${seeded.photoCount})`,
    ).toBeLessThan(seeded.photoCount);
  });
});
