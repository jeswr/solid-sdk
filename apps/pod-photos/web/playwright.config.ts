...
//
// Playwright config for the Pod Photos static host's e2e layer. The first (and
...
// attaches the DPoP token UP FRONT — so a gallery of N photo resources pays ZERO
// wasted 401s, never the per-resource 401-dance. (pod-photos' listGallery reads
// EACH photo document individually after listing the container, so N photos =>
...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  // The 401-budget spec drives a real OIDC popup login + a gallery listing, so it
