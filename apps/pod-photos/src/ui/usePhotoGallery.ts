...
//
// The photo-gallery's data hook — the SINGLE place the view touches the data
// layer. It owns the "which container am I viewing + its loading/error state"
// concern and delegates the actual GET+parse to `listGallery` (src/ui/gallery.ts,
// which itself composes the data-layer primitives); it never re-implements
// LDP/RDF reading.
//
// ── AUTH SEAM ────────────────────────────────────────────────────────────────
...
import { GalleryAccessError, type GalleryListing, listGallery } from './gallery.js';
 ...
 * state — keeping `currentUrl` (and therefore the breadcrumb derivation) in step
 * with the URL the data layer actually GETs.
 */
function ensureTrailingSlash(url: string): string {
  return url.endsWith('/') ? url : `${url}/`;
}

/** What the view needs to render a gallery + its breadcrumb + states. */
...
/** Options for {@link usePhotoGallery}. */
...
/**
 * React state for browsing a Solid photo-container tree. `rootUrl` is normalised
 * to a single trailing slash; the hook loads it on mount, whenever the caller
 ...
 * All reads go through `listGallery`, so WAC handling, slash normalisation, and
 * the typed model come for free.
 */
export function usePhotoGallery(
  ...
  // a slashless `rootUrl` (e.g. ".../photos") still drives a slash-terminated
  // container GET and a correct breadcrumb. The raw prop is never used directly.
  ...
  // Reset navigation + listing state DURING render when the normalised root prop
  // changes (React's documented "adjusting state when a prop changes" pattern —
  // applies in the same commit, so the view never flashes the previous container
  // or its stale breadcrumb). The load effect below then GETs the new root. The
  // mount case is excluded because `prevRoot` is seeded with the initial root.
  // This is driven by the `prevRoot` STATE, not a render-time ref write, so it is
  // correct under React's concurrent rendering.
  ...
  useEffect(() => {
    ...
    listGallery(currentUrl, {
      ...
      .catch((err: unknown) => {
        ...
        // we never surface an error or state from a request that is no longer
        // current.
        if (requestId !== requestIdRef.current) {
          return;
        }
        if (err instanceof GalleryAccessError) {
          setIsAccessError(true);
          setError(
            err.status === 401
              ? 'You need to log in to view these photos.'
    ...
    return () => {
      // Mark any in-flight response as stale and abort the underlying walk.
