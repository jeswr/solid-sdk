...
// Vendored from solid-pod-manager src/components/sidebar-nav.tsx
 ...
 * For query-param-based views (href includes "?view=") we match both the
 * pathname and the view param; the root "/" with no param matches the issues
 * list (view=list or no view param).
 */
function isActive(pathname: string, searchParams: URLSearchParams, href: string): boolean {
  const [hrefPath, hrefQuery] = href.split("?");
  if (pathname !== (hrefPath || "/")) return false;
  if (!hrefQuery) {
    // "/?view=..." items should NOT match the bare "/" entry
    return !searchParams.has("view");
  }
  const hrefParams = new URLSearchParams(hrefQuery);
  const view = hrefParams.get("view");
  return view !== null && searchParams.get("view") === view;
}
