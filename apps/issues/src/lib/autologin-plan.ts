...
// out of SolidSessionProvider so the security-critical scenarios are testable with
// NO browser / React / oauth stack (the vitest env here is `node`, and the test
// suite is `src/**/*.test.ts` only). Ported from pod-docs' reference
// autologin-plan.ts and adapted to solid-issues' SessionStatus state machine.
//
...
export interface AutologinInputs {
  ...
  hasProvider: boolean;
  /** Already authenticated — a session WINS; autologin is skipped. */
  loggedIn: boolean;
   ...
   * ({@link webIdsEqual}). Injected so this pure module stays free of the provider
   * import and the test pins the exact equality. The loop guard compares the
   * sentinel to `fragmentWebId` through this, so a stale sentinel for a DIFFERENT
   * WebID does not swallow a fresh deep-link.
   */
