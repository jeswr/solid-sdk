...
//
// Vite config for the Pod Drive static host shell. It bundles the LOCAL
// `@jeswr/pod-drive/ui` React source directly (no pre-built dist/ui needed —
...
// Resolve the @jeswr/pod-drive library to its LOCAL TypeScript SOURCE, so Vite
...
const podDriveSrc = (rel: string) => fileURLToPath(new URL(`../src/${rel}`, import.meta.url));
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      { find: "@jeswr/pod-drive/ui", replacement: podDriveSrc("ui/index.ts") },
      { find: "@jeswr/pod-drive", replacement: podDriveSrc("index.ts") },
    ...
    dedupe: [
      ...
      // (transitive) deps of solid-elements; pin them to a single copy.
      "lit",
      "@lit/react",
    ],
  },
