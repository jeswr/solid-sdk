...
//
// Playwright config for the Pod Drive static host's e2e layer. The first (and
...
// attaches the DPoP token UP FRONT — so a container listing of N children pays
// ZERO wasted 401s, never the per-resource 401-dance.
//
...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  // The 401-budget spec drives a real OIDC popup login + a container listing, so it
  ...
  webServer: [
    {
      // In-memory WAC CSS, pinned major 7 (the suite-standard local Solid server).
      ...
      // (the roborev LOW finding). A fresh in-memory CSS per run guarantees clean state
      // locally AND in CI; the ~13s boot is paid once per `playwright test` invocation.
