...
// @jeswr/app-shell <FeedbackButton/> renders with pod-drive's OWN repo
// (`jeswr/pod-drive`) and app name, and opening it surfaces the themed feedback
...
// the load-bearing, app-specific contract here is "files against jeswr/pod-drive".
//
...
describe("header FeedbackButton (pod-drive adoption)", () => {
  it("renders the Feedback trigger with repo=jeswr/pod-drive wiring", () => {
    render(
      <FeedbackButton
  ...
  it("opens the feedback dialog targeting the app's own repo", async () => {
    ...
    // GitHub URL — built from repo="jeswr/pod-drive" — is unit-tested in
    // app-shell's own buildIssueUrl suite.)
    const dialog = await screen.findByRole("dialog");
    expect(dialog).toBeInTheDocument();
    expect(dialog).toHaveTextContent(/Pod Drive/i);
  });
});
