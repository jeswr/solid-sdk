...
// The forbidden transports. hosted-git-info rewrites a GitHub git dep to one of
// three SSH forms, ALL of which require an SSH key and so break keyless `npm ci`:
//   1. `git+ssh://git@github.com/owner/repo`  (the form npm 11.x writes to lockfiles)
//   2. `ssh://git@github.com/owner/repo`      (the bare ssh:// URL form)
//   3. `git@github.com:owner/repo`            (the scp-like shorthand — also SSH, and
//      may appear with an optional `git+` prefix, e.g. `git+git@github.com:...`)
// Match all three. (We only flag GitHub here, which is where every @jeswr github:
// dep lives — the #78 surface.)
const FORBIDDEN = [
  /git\+ssh:\/\/git@github/,
  /(^|["@/])ssh:\/\/git@github/,
  // scp-style `git@github.com:owner/repo` (optionally `git+`-prefixed). Anchor the
  // leading `git@` on a non-word boundary (string start or a JSON `"` / `+`) so it
  // does not match an email-like `git@github` substring inside a longer token.
  /(^|["+])git@github\.com:/,
];
