...
//
// Vitest config for the Pod Drive static host. It MERGES the Vite build config
// (vite.config.ts) so the test run inherits the same resolve.alias (the local
// @jeswr/pod-drive source + the single-React dedupe) and the `__APP_VERSION__`
...
export default mergeConfig(
  ...
  defineConfig({
    test: {
      ...
      // Both the src component/logic tests AND the `scripts/` gen-clientid suite
      // (a plain `.test.mjs` — it sits in scripts/ to stay out of tsc; see the
      // file header). Adding `scripts/**/*.test.mjs` keeps it in `npm test`:
      // before this config the bare `vitest run` used the default include and ran
      // it, so narrowing to only `src/**` would have silently dropped its coverage.
