...
//
// Exhaustive, security-critical tests for the PURE silent-session-restore
// decision (no browser, no oauth stack — the one fetch is injected). The branch
// table this pins: nothing remembered → login; no remembered issuer → login;
// refresh grant succeeds → restored; refresh grant returns undefined
// (expired/revoked) → login; an UNEXPECTED throw → login (fail-closed); a restored
