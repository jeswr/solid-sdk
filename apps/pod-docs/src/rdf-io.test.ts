...
const URL_X = "https://alice.pod/pod-docs/x.ttl";

describe("serializeTurtle", () => {
  it("serialises a dataset to Turtle with prefixes", async () => {
    const store = turtleToStore(
      `@prefix dct: <http://purl.org/dc/terms/> . <#it> dct:title "Hi" .`,
      URL_X,
    );
    const ttl = await serializeTurtle(store, { dct: "http://purl.org/dc/terms/" });
...
describe("readRdf", () => {
  it("reads + parses a resource and sends the revalidate header", async () => {
    const { fetch, calls } = mockFetch({
      [`GET ${URL_X}`]: {
        body: `@prefix dct: <http://purl.org/dc/terms/> . <#it> dct:title "T" .`,
        etag: 'W/"abc"',
      },
    });
  ...
  it("uses the global fetch when no fetchImpl is given (production path)", async () => {
    ...
    globalThis.fetch = (async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(`@prefix dct: <http://purl.org/dc/terms/> . <#it> dct:title "G" .`, {
        status: 200,
        headers: { "content-type": "text/turtle", etag: 'W/"g"' },
      });
    }) as typeof fetch;
    try {
...
describe("nameFromUrl", () => {
  it("returns the last decoded path segment", () => {
    expect(nameFromUrl("https://pod/pod-docs/my%20note.ttl")).toBe("my note.ttl");
  });
