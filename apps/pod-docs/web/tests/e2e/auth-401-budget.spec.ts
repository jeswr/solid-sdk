...
// storage root and NEVER a count that scales with the document count.
//
// REGRESSION SURFACE — pod-docs loads the document list by LISTING the documents
// container AND THEN READING EACH child document (DocsStore.list → read per child, see
// src/store.ts), and the App ALSO reads the WebID profile — so a container of N documents
// drives N+1 DISTINCT pod reads on load. Under the old reactive manager each of those
// reads paid its own 401; the proactive patch keeps the resource-server 401 count FLAT
// (≤1 per storage root), never scaling with the document count. The DOCUMENT COUNT (one
// `.pod-docs-row` per document) is the regression surface.
//
...
//   • Wait for the document list to render its N rows (login lands on the browser) and
//     assert the resource-server 401 count is bounded and does NOT scale with the count.
...
/** Drive the WebID-first popup login. Returns once the document browser is on screen.
 *
...
async function login(page: Page, seeded: Seeded): Promise<boolean> {
  ...
  // Back on the app: the document browser appears once logged in.
  await expect(page.locator(".pod-docs-browser")).toBeVisible({ timeout: 60_000 });
  return true;
}

test.describe("401-budget — the proactive auth-fetch eliminates the 401-dance", () => {
  test("a session loads the documents paying ≤1 resource-server 401 per storage root", async ({
    page,
  }) => {
    ...
    // The browser loads its document rows on login. DocsStore.list reads the container
    // and EACH child document, rendering one `.pod-docs-row` per `pd:Document`. Wait for
    // the N rows to render, proving we actually loaded every document.
    await expect
      .poll(async () => page.locator(".pod-docs-table tbody tr.pod-docs-row").count(), {
        timeout: 30_000,
      })
      .toBeGreaterThanOrEqual(seeded.documentCount);
    ...
    // (c) REGRESSION GUARD: the 401 count does NOT scale with the document count. Under
    //     the old reactive manager every distinct pod document read on load paid its own
    //     401 (the container listing + each child GET), so a larger container pushes
    //     `total` upward. The proactive patch keeps it flat — strictly below the document
    //     count (which is ≤ the number of pod documents the app reads on load).
    expect(
      total,
      `401 count (${total}) must not scale with document count (${seeded.documentCount})`,
    ).toBeLessThan(seeded.documentCount);
  });
});
