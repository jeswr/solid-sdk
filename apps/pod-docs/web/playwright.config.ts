...
//
// Playwright config for the Pod Docs static host's e2e layer. The first (and
...
// attaches the DPoP token UP FRONT — so loading the documents (which lists the
// documents container AND reads each child document — N+1 distinct pod reads) pays
// ZERO wasted 401s, never the per-resource 401-dance the old reactive manager incurred
// (one wasted 401 per pod document read).
//
...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  // The 401-budget spec drives a real OIDC popup login + a document listing, so it
  ...
  webServer: [
    {
      ...
      // ~635 packages, so vendoring it into every pod-app's node_modules would balloon
      // disk for an e2e-only, ephemeral server — the suite's disk-hygiene rule. `npx`
      // caches it in the shared npm cache after first fetch.
