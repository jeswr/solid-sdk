...
// @jeswr/app-shell <FeedbackButton/> renders with pod-docs's OWN repo
// (`jeswr/pod-docs`) and app name, and opening it surfaces the themed feedback
...
// jeswr/pod-docs for exactly the config the header passes.
//
...
describe("header FeedbackButton (pod-docs adoption)", () => {
  it("the App's feedback config targets jeswr/pod-docs", () => {
    // Load-bearing, app-specific contract: a filed issue must land on THIS repo.
    expect(FEEDBACK_REPO).toBe("jeswr/pod-docs");
    ...
    expect(url).toContain("github.com/jeswr/pod-docs/issues/new");
  });
  ...
  it("opens the feedback dialog targeting the app's own repo", async () => {
    ...
    expect(dialog).toHaveTextContent(/Pod Docs/i);
  });
});
