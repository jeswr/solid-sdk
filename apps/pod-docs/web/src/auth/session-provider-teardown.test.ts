...
//
// THE BUG (roborev HIGH, surfaced by pod-health, back-ported to pod-docs): the #123
// proactive-fetch rollout left the AUTHORITATIVE credential-boundary arm + the UI publish in
// `establishSessionFor` UNFENCED — they armed + published UNCONDITIONALLY after the async
// profile re-read (`readProfile`). A logout() or new login() that RACES that await can (a)
// re-arm the boundary against a reset provider behind a logged-out UI, (b) republish a stale
// webId/session, or (c) clear a NEWER login's boundary. The fix fences the authoritative arm +
// publish with the pure, exported `establishStillCurrent` (generation unchanged + WebID
// matches); on the SUPERSEDED path the caller BAILS WITHOUT clearing the boundary (clearing
// would wipe a newer login's boundary).
//
// NOTE (pod-docs delta): pod-docs has NO async issuer accessor and NO silent-restore, so the
// PROVISIONAL arm runs in the same synchronous tick as the entry WebID check (no fence needed
// there — no await separates them); the ONLY awaited step in establishSessionFor is the profile
// re-read, and everything after it is fenced.
//
...
// proactive-boundary arm + UI publish in establishSessionFor (roborev HIGH). A logout()/new
// login() racing the profile re-read advances the generation (via reset()) AND clears the
// boundary; without this fence the resumed establish would RE-ARM the boundary against a
// reset/stale provider AND publish a stale logged-in session — or, on a NEW login, clobber the
// new login's freshly-armed boundary.
describe("roborev HIGH — establishStillCurrent (fences the authoritative arm + publish)", () => {
  const GEN = 7;
  ...
  it("REJECTION PATH — a superseded establish whose profile-read THREW must also fail-closed (roborev MEDIUM, 2nd round)", () => {
    // establishSessionFor fences the profile-read REJECTION with the SAME establishStillCurrent
    // check it uses on success: if `readProfile` THROWS after a racing logout/new-login (B)
    // superseded this attempt, the catch must SWALLOW the error and return WITHOUT clearing the
    // boundary — letting the rejection propagate to doLogin's catch would reset() + clear B's
    // freshly-armed boundary and surface a stale error from the obsolete attempt. The decision
    // is identical to the success fence: superseded ⇒ false ⇒ swallow+return.
    expect(
      establishStillCurrent({
        establishGeneration: GEN, // A's snapshot
        currentGeneration: GEN + 1, // B advanced it while A's profile read was failing
        requestedWebId: A,
        currentAuthenticatedWebId: B, // B is now authenticated
        webIdsEqual,
      }),
    ).toBe(false);
    // …and a still-current rejection (no race) returns true ⇒ the catch RE-THROWS the genuine
    // profile-read failure so the real login error surfaces (the non-superseded branch).
    expect(
      establishStillCurrent({ ...base, currentGeneration: GEN, currentAuthenticatedWebId: A }),
    ).toBe(true);
  });
