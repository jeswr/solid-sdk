...
export class WebIdDPoPTokenProvider implements TokenProvider {
   ...
   * (`#profileFetch`, captured before any global patch), NOT the patched `globalThis.fetch`.
   * The proactive auth-fetch patch (@jeswr/solid-elements/auth) calls THIS provider's
   * `upgrade()` for any allowed-origin request — and the OIDC endpoints live on the issuer
   * origin, which IS in the credential boundary. Without pinning `customFetch`, an internal
   * token-endpoint request would re-enter the patched global → call `upgrade()` again →
   * recurse / deadlock login, and would clobber the token-endpoint's own `Authorization`
   * (client-credentials / none) with the resource DPoP token. `#profileFetch` is the
   * un-patched native fetch, so these requests never re-enter the wrapper.
   */
  #httpOptions(
    issuer: URL,
    signal: AbortSignal,
