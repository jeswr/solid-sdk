...
// Scan from the GIT REPO ROOT, not the cwd, so the guard genuinely covers EVERY
// committed lockfile (the repo-root `package-lock.json` AND the nested `web/`
// one) as its header promises — `npm run check:lockfile-transport` runs from
// `web/`, so a bare `process.cwd()` would miss the root lockfile entirely
// (roborev finding). Fall back to the cwd when git is unavailable (e.g. a tarball
// build) so the guard never hard-fails on its own root resolution.
function repoRoot() {
  try {
