...
//
// Vite config for the Pod Docs static host shell. It bundles the LOCAL
// `@jeswr/pod-docs/ui` React source directly (no pre-built dist/ui needed —
...
// Resolve the @jeswr/pod-docs library to its LOCAL TypeScript SOURCE, so Vite
...
const podDocsSrc = (rel: string) => fileURLToPath(new URL(`../src/${rel}`, import.meta.url));
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      { find: "@jeswr/pod-docs/ui", replacement: podDocsSrc("ui/index.ts") },
      { find: "@jeswr/pod-docs", replacement: podDocsSrc("index.ts") },
