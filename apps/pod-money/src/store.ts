...
export class MoneyStore {
   ...
   * document (the resource simply does not exist yet) with `etag: null`, so a
   * caller can mint into it and PUT to create. Any other failure re-throws.
   */
  ...
  /**
   * Conditionally write a dataset back to `url` as Turtle. Sends `If-Match`
   * when an ETag is supplied (lost-update-safe); on a missing ETag (resource
   * does not exist yet, or a server that emits none) sends `If-None-Match: *`
   * so a create never clobbers a concurrently-created resource. A 412 surfaces
   * as a `PreconditionFailedError` for the caller to re-read and retry.
   */
  async save(url: string, dataset: DatasetCore, etag: string | null): Promise<Response> {
    ...
    if (response.status === 412) {
      throw new PreconditionFailedError(url, etag);
    }
    if (!response.ok) {
   ...
   * Conditional on the read ETag. Returns the write `Response`.
   */
  async addAccount(account: NewAccount): Promise<Response> {
    const { document, dataset, etag } = await this.loadLedger();
    ...
    return this.save(this.ledgerUrl, dataset, etag);
  }

  /**
   * Read-modify-write a new transaction (and its monetary-amount node) into the
   * ledger and PUT it back. Conditional on the read ETag.
   */
   ...
   * type index, adds an idempotent registration, and PUTs it back.
   */
  async registerInTypeIndex(): Promise<Response> {
    const url = this.publicTypeIndexUrl;
    const { dataset, etag } = await this.load(url);
    ...
    return this.save(url, dataset, etag);
  }
