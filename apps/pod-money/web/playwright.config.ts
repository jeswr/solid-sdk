...
//
// Playwright config for the Pod Money static host's e2e layer. The first (and
...
// attaches the DPoP token UP FRONT — so pod-money's LEDGER-DISCOVERY CHAIN (the
// authenticated profile re-read + the public-type-index read + the ledger GET) pays
// ZERO wasted 401s, never the per-resource 401-dance. (Each of those is a DISTINCT
// pod URL that paid its own wasted 401 under the reactive manager.)
//
...
//
// LOCAL-ONLY: this NEVER targets the live deploy (solid-test.jeswr.org). All auth +
...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  // The 401-budget spec drives a real OIDC popup login + a ledger load, so it
  ...
  webServer: [
    {
      ...
      // repo change. We deliberately keep `npx` (not a declared devDependency): CSS pulls
      // ~635 packages, so vendoring it into every pod-app's node_modules would balloon disk
      // for an e2e-only, ephemeral server — the suite's disk-hygiene rule. `npx` caches it
      // in the shared npm cache after first fetch. reuseExistingServer:false ALWAYS —
      // global-setup seeds a FIXED `alice` account/pod, so a reused (stateful) CSS from a
      // prior run would make the second pod-create 4xx. A fresh in-memory CSS per run
      // guarantees clean state locally AND in CI; the ~13s boot is paid once per `playwright
      // test` invocation.
