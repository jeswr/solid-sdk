...
// storage root and NEVER a count that scales with the data size.
//
// REGRESSION SURFACE — UNLIKE pod-music (which reads N PER-TRACK resources), pod-money
// reads its accounts + transactions from a SINGLE ledger file. So the 401-dance surface
// here is the DISCOVERY CHAIN it walks on login, each a DISTINCT pod URL:
//   1. the authenticated profile re-read (`establishSessionFor` → `readProfile`);
//   2. the public Type Index (`MoneyStore.discover` → `settings/publicTypeIndex.ttl`);
//   3. the ledger file GET (`readLedger` → `finance/ledger.ttl`).
// Under the old reactive manager EACH of these paid a wasted 401; the proactive patch
// keeps the 401 count flat (≤1 per storage root). The seeded transaction COUNT is the
// regression knob — it adds TRIPLES to one file, never reads, so the 401 count must NOT
// scale with it.
//
...
//   • Wait for the accounts list to render (login lands on the AccountsView) and assert
//     the resource-server 401 count is bounded and does NOT scale with the data size.
...
/** Drive the WebID-first popup login. Returns once the accounts view is on screen.
 *
...
async function login(page: Page, seeded: Seeded): Promise<boolean> {
  ...
  // Back on the app: the accounts view appears once logged in (after the ledger discovery
  // resolves and AccountsView renders its `.pod-money-accounts` root).
  await expect(page.locator(".pod-money-accounts")).toBeVisible({ timeout: 60_000 });
  return true;
}

test.describe("401-budget — the proactive auth-fetch eliminates the 401-dance", () => {
  test("a session loads the ledger paying ≤1 resource-server 401 per storage root", async ({
    page,
  }) => {
    ...
    // FAIL-CLOSED on a non-completing popup login (roborev MEDIUM, back-ported from
    // pod-health becddf5): a non-completing popup is a genuine regression (e.g. the
    // <authorization-code-flow> element failing to register, or the popup never opening), so
    // by DEFAULT we FAIL — never silently mask it. The skip is allowed ONLY when the explicit
    // opt-in (`ALLOW_E2E_POPUP_SKIP=1`) is set AND we are NOT in CI — a constrained local
    // worktree where the headless reactive-auth dialog→window.open flow is known-flaky. CI is
    // AUTHORITATIVE: `process.env.CI` forces a hard FAILURE even if the opt-in is mistakenly
    // set in the CI environment, so a misconfigured CI can never pass without exercising the
    // only e2e assertion for this change. The proactive-fetch behaviour is independently
    // covered by the unit tests (the webid-token-provider re-entrancy guard) + the
    // @jeswr/solid-elements unit tests either way.
    if (!loggedIn) {
      // STRICT opt-in (roborev Low): only the exact value "1" enables the skip — so a stray
      // `ALLOW_E2E_POPUP_SKIP=0` / `=false` does NOT accidentally skip the test outside CI.
    ...
    // The login lands on the AccountsView, which loads the ledger (the discovery chain:
    // the authenticated profile re-read + the type index + the ledger GET). Wait for the
    // seeded account row to render, proving the ledger was actually read + parsed.
    await expect
      .poll(async () => page.locator(".pod-money-account-row").count(), { timeout: 30_000 })
    ...
    // (c) REGRESSION GUARD: the 401 count does NOT scale with the data size. Under the
    //     old reactive manager every distinct pod URL in the discovery chain (profile,
    //     type index, ledger) paid its own 401, so `total` tracked the chain length. The
    //     proactive patch keeps it flat — strictly below the seeded transaction count
    //     (≥ 12), which is comfortably above the (fixed, small) discovery-chain length.
    expect(
      total,
      `401 count (${total}) must not scale with the seeded data size (${seeded.txnCount} txns)`,
    ).toBeLessThan(seeded.txnCount);
  });
});
