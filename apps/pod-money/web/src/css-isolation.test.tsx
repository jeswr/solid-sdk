...
// rollout #67/#68/#70).
//
// pod-money RELAXED its CSS-leak workaround back to a BARE global `button {}` filled
// rule + a direct re-alias of the host's `--accent` / `--accent-ink` onto the
// app-shell tokens (dropping the earlier `@layer base` wrapper + the `--host-accent`
// renaming), because @jeswr/app-shell #80 made the shell SELF-ISOLATING. This test
// pins WHY that relaxation is safe so a future app-shell bump that regressed the
// isolation would be caught here, not in production:
//
...
describe("app-shell CSS isolation survives pod-money's bare button {} (#80)", () => {
  it("every header control App renders is isolation-tagged", () => {
    ...
    const { container } = render(
      ...
      <ThemeProvider>
        <FeedbackButton repo="jeswr/pod-money" appName="Pod Money" appVersion="testsha" />
        <ThemeToggle />
        <AccountMenu
  ...
  it("every control in the portaled feedback dialog is isolation-tagged", async () => {
    render(<FeedbackButton repo="jeswr/pod-money" appName="Pod Money" />);
  ...
  // to (0,1,1), out-ranking the host's CLASS-only overrides (`.pod-money-account-link`,
  // `.pod-money-back-link`) and repainting those filled. `:where()` carries ZERO
  // specificity, so the base stays (0,0,1) — identical to the original bare
  // `button {}` — and the host overrides keep winning. jsdom does not run the cascade,
  // so we pin the selector form at the SOURCE (the contract the cascade math relies on).
  it("scopes the host button base with zero-specificity :where() (box-model + overrides safe)", () => {
    ...
    // And it is NOT the bare specificity-raising form (that would repaint the host
    // link/outline buttons). Match the SELECTOR position only (`button:not(...) {`),
    // so the explanatory comment that names the rejected form does not trip this guard.
