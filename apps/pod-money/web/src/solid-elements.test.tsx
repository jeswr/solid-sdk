...
describe("@jeswr/solid-elements <Loading> (pod-money adoption)", () => {
  ...
  it("mounts the jeswr-loading custom element via the @lit/react adapter", async () => {
    const { container } = render(<Loading label="Finding your finance ledger…" />);
  ...
  it("shows the contextual label text + reflects it (df0fbe4 reflect:true)", async () => {
    ...
    await customElements.whenDefined("jeswr-loading");
    el.label = "Finding your finance ledger…";
    ...
    expect(labelSpan?.textContent).toBe("Finding your finance ledger…");
    // (2) df0fbe4 reflects the string property onto the host attribute.
    expect(el.getAttribute("label")).toBe("Finding your finance ledger…");
