...
//
// Vite config for the Pod Money static host shell. It bundles the LOCAL
// pod-money `./ui` React source directly (no pre-built dist/ui needed —
...
// Resolve the pod-money library to its LOCAL TypeScript SOURCE, so Vite compiles
// `../src/ui` (and the data-layer core) directly — no pre-built dist/ui is needed,
// and the host always tracks the library's current source. The source uses
// NodeNext-style `.js` extension imports (e.g. `./format.js`); Vite's resolver
// maps those to the sibling `.ts` files when bundling.
const podMoneySrc = (rel: string) => fileURLToPath(new URL(`../src/${rel}`, import.meta.url));

// The library SOURCE lives OUTSIDE web/ (in ../src), so when Rollup bundles it,
// its bare imports (@jeswr/fetch-rdf, @solid/object, @rdfjs/wrapper, n3, …) would
// resolve against ../src's nearest node_modules — which doesn't exist. Pin those
// runtime deps to THIS host's node_modules so the out-of-root source resolves them.
...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: [
      { find: "@jeswr/pod-money/ui", replacement: podMoneySrc("ui/index.ts") },
      { find: "@jeswr/pod-money", replacement: podMoneySrc("index.ts") },
    ...
    // (git+https://github.com/jeswr/app-shell.git, pinned sha, committed dist/) —
    // but a package can still resolve react/react-dom + Radix/lucide against its
    // OWN nested node_modules, pulling a SECOND copy (invalid-hook-call). Deduping
    // these forces ONE copy from the host's node_modules (where they are installed
    // as direct deps). react/react-dom are also alias-pinned above for the bundled
    // library.
    dedupe: [
      "react",
      "react-dom",
