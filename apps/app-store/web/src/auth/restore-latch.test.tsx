...
describe("FINDING 4 (implementation) — runtime-init rejection un-sticks the Restoring UI", () => {
  afterEach(() => {
    cleanup();
    localStorage.removeItem("solid-app-store:remembered-account");
    invalidateSilentRestoreLatch();
  });

  it("ENTERS the restoring state then is un-stuck when getAuthRuntime rejects", async () => {
    ...
    // The module key matches SessionProvider's RememberedAccount namespace for this app.
    localStorage.setItem(
      "solid-app-store:remembered-account",
    ...
    expect(new RememberedAccount("solid-app-store:remembered-account").read()).not.toBeNull();
    ...
    // indicator is painted. This is the assertion that makes the test NON-VACUOUS: a test
    // that never entered the restoring state would fail HERE. (The runtime-init rejection
    // is a microtask-deferred chain off the dynamic import, so it has not yet propagated.)
    //
    // STORE UI NOTE: unlike the pod-apps (which gate the whole page on a
    // `main[aria-busy="true"]` restoring screen), the App Store always renders its catalog
    // and shows the restoring state as an INLINE header indicator — a `.store-restoring`
    // span containing the <jeswr-loading> Web Component — that swaps to the "Sign in"
    // button once restore settles. So the FINDING-4 signal here is that inline indicator.
    //
    // SOLID-ELEMENTS NOTE (#115 adoption): @lit/react does NOT forward the `label` PROPERTY
    // into reflected text under React 19 + jsdom (a documented adapter rough edge — see
    // solid-elements.test.tsx), so we anchor on the LABEL-INDEPENDENT signal: the
    // `.store-restoring` span with a mounted `<jeswr-loading>` inside it, and the absence of
    // the "Sign in" button while restoring. (A real browser DOES show the label.)
    const restoringIndicator = document.querySelector(".store-restoring");
    expect(restoringIndicator).not.toBeNull();
    expect(restoringIndicator?.querySelector("jeswr-loading")).not.toBeNull();
    // While restoring, the Sign-in button has not yet replaced the indicator.
    expect(document.querySelector("button.store-signin")).toBeNull();

    // POSTCONDITION — the catch flipped restoringSession off, so the restoring indicator is
    // gone and the Sign-in control renders (no session was restored — the runtime failed).
    // If `setRestoringSession(false)` is removed from the production runtime-init `.catch`,
    // `ready` stays false forever, the restore effect never runs, and the restoring
    // indicator NEVER disappears → this waitFor times out and the test FAILS (the
    // adversarial check). We assert the inline restoring indicator (its mounted
    // <jeswr-loading>) is gone AND the Sign-in button has appeared — both label-independent.
    await waitFor(
      () => {
        expect(document.querySelector(".store-restoring jeswr-loading")).toBeNull();
        expect(document.querySelector("button.store-signin")).not.toBeNull();
      },
      { timeout: 4000 },
    );
  });
